...
import { detectInitialLanguage, formatDate, formatMoney, getCurrencySymbol, normalizeCurrency, normalizeLanguage, translations } from './i18n'

const appCopy = translations.app
...
function App() {
  ...
  const [currency, setCurrencyState] = useState(() => normalizeCurrency(localStorage.getItem('currency')))
  const [showLanguagePrompt, setShowLanguagePrompt] = useState(() => !localStorage.getItem('language'))
  ...
  const t = appCopy[language] || appCopy.tr
  const money = useCallback((value, options = {}) => formatMoney(value, { language, currency, ...options }), [language, currency])
  const dateLabel = useCallback((value, options = {}) => formatDate(value, { language, ...options }), [language])
  ...
  const syncToSupabase = async (newData, newLimits) => {
    ...
    try {
      ...
      const addFamilyIdToTransactions = (items) => items.map(item => {
        const normalized = { ...toSnakeCase([item])[0], family_id: familyId }
      ...
      if (newData.transactions) await supabase.from('transactions').upsert(addFamilyIdToTransactions(newData.transactions))
  ...
  */
  ...
  const expenseCategories = [
    { id: 'market', tr: 'Market', en: 'Groceries', keywords: ['market', 'migros', 'carrefour', 'sok', 'şok', 'bim', 'a101', 'grocery'] },
    { id: 'transport', tr: 'Ulaşım', en: 'Transport', keywords: ['benzin', 'mazot', 'shell', 'opet', 'bp', 'otobus', 'otobüs', 'metro', 'taksi', 'taxi', 'fuel'] },
    { id: 'bill', tr: 'Fatura', en: 'Bills', keywords: ['elektrik', 'su', 'dogalgaz', 'doğalgaz', 'internet', 'telefon', 'fatura', 'bill'] },
    { id: 'rent', tr: 'Kira', en: 'Rent', keywords: ['kira', 'rent'] },
    { id: 'subscription', tr: 'Abonelik', en: 'Subscription', keywords: ['netflix', 'spotify', 'youtube', 'abonelik', 'subscription', 'prime', 'disney'] },
    { id: 'health', tr: 'Sağlık', en: 'Health', keywords: ['eczane', 'hastane', 'doktor', 'ilaç', 'ilac', 'pharmacy', 'hospital'] },
    { id: 'education', tr: 'Eğitim', en: 'Education', keywords: ['okul', 'kurs', 'kitap', 'egitim', 'eğitim', 'school', 'course'] },
    { id: 'entertainment', tr: 'Eğlence', en: 'Entertainment', keywords: ['sinema', 'cafe', 'kahve', 'restoran', 'restaurant', 'oyun', 'game'] },
    { id: 'other', tr: 'Diğer', en: 'Other', keywords: [] }
  ]

  const inferTransactionCategory = (text = '', type = '') => {
    if (type === 'gelir') return 'income'
    if (['abonelik', 'subscription', 'fixed', 'debt'].includes(type)) return 'subscription'
  ...
  const getStoredTransactionCategory = (transaction) => {
    const marker = transaction?.description?.match(/\[kategori:([^\]]+)\]/)?.[1]
    return transaction?.category || marker || inferTransactionCategory(transaction?.description || '', transaction?.type)
  }

  const withCategoryMarker = (text, categoryId) => {
    const clean = getDisplayDescription(text)
    const category = categoryId || inferTransactionCategory(clean)
    return `${clean} [kategori:${category}]`
  }

  const getCategoryMeta = (categoryId, type = '') => {
    if (type === 'gelir' || categoryId === 'income') {
      return { id: 'income', label: language === 'en' ? 'Income' : 'Gelir', className: 'bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-300 border-green-100 dark:border-green-800/50' }
    }
    const category = expenseCategories.find(item => item.id === categoryId) || expenseCategories.find(item => item.id === 'other')
    return {
      id: category.id,
      label: language === 'en' ? category.en : category.tr,
  ...
  useEffect(() => {
    if (!categoryTouched) {
      setTransactionCategory(inferTransactionCategory(description))
    }
      ...
      subscriptions
        ...
        .forEach(item => {
          ...
          occurrences.push({
            ...
            category: inferTransactionCategory(item.name, item.paymentType || 'subscription'),
  ...
  const getCashFlowForRange = (startDate, endDate, userId = 'all') => {
    ...
    const realTransactions = activeTransactions
      ...
      .map(t => ({
        ...
        amount: t.amount,
        type: t.type,
        category: getStoredTransactionCategory(t),
  ...
  const handleEditTransaction = (t) => {
    ...
    const cleanDescription = getDisplayDescription(t.description).replace(/ \(\d+\/\d+\)$/, '').replace(/ \(\d+ Taksit\)$/, '')
    setDescription(cleanDescription) // Strip installment info for pure edit if possible, or just strict edit
    setTransactionCategory(getStoredTransactionCategory(t))
    setCategoryTouched(Boolean(t.category))
  ...
  const handleShareWhatsApp = () => {
    ...
    filteredTransactions.forEach(t => {
      const date = dateLabel(t.date, { day: '2-digit', month: '2-digit' });
    ...
    message += `--------------------------------\n`;
    message += `💰 *TOPLAM: ${money(totalAmount)}*`;
      ...
      <div className="absolute inset-0 z-[90] flex items-end sm:items-center justify-center pointer-events-none overflow-x-hidden">
        ...
        <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl w-full max-w-full sm:w-[420px] max-h-[90vh] rounded-t-[40px] sm:rounded-[40px] p-6 sm:p-8 relative z-10 animate-slide-up shadow-2xl flex flex-col pointer-events-auto border border-white/50 dark:border-slate-800/50 transition-colors overflow-x-hidden">
          ...
          <form onSubmit={handleAddIncome} className="space-y-4 mb-6">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wide pl-2">{t.account}</label>
              <select className="w-full p-4 bg-gray-50/50 dark:bg-slate-800 text-gray-800 dark:text-white font-bold rounded-2xl border border-gray-200 dark:border-slate-700 appearance-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none transition-all cursor-pointer" value={incomeAccount} onChange={e => setIncomeAccount(e.target.value)}>
                {selectableAccounts.length === 0 && <option value="">{language === 'en' ? 'Add an account first' : 'Önce hesap ekleyin'}</option>}
  ...
  const handleExportData = () => {
    const exportPayload = {
      ...
      currency,
      family: {
        id: profile?.family_id || null,
        name: profile?.families?.name || null,
        inviteCode: profile?.families?.invite_code || null
      },
      users: data.users || [],
    ...
    link.href = url;
    link.download = `aiekonomi-export-${new Date().toISOString().slice(0, 10)}.json`;
          ...
          <div className="absolute inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl w-full max-w-[360px] rounded-[40px] p-8 relative z-10 animate-scale-up shadow-2xl border border-white/50 dark:border-slate-800/50">
              <div className="text-center mb-6">
                ...
                <h3 className="text-2xl font-black text-gray-800 dark:text-white mb-3">{t.success}</h3>
                <p className="text-base text-gray-600 dark:text-gray-400 leading-relaxed">
                  {successMessage}
                </p>
              </div>
  ...
  if (currentView === 'cashflow') {
    const formatCurrencyCompact = (value) => money(value, { maximumFractionDigits: 0 })
    ...
    const formatCashFlowDate = (dateKey) => {
      ...
      if (dateKey === formatDateKey(tomorrow)) return t.tomorrow
      return dateLabel(date, { day: 'numeric', month: 'long' })
    }
    ...
    const cashFlowStatus = cashFlowSelectedRange.net >= 0
      ? { label: t.comfortable, className: 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300' }
      : { label: t.deficit, className: 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-300' }
    ...
    return (
      <div className="fixed inset-0 bg-[#F2F4F8] dark:bg-slate-950 z-[100] flex flex-col animate-in slide-in-from-right duration-300">
        <header className="px-6 md:px-8 pt-[calc(1rem+var(--safe-area-inset-top))] pb-4 flex items-center gap-4 bg-[#F2F4F8] dark:bg-slate-950 border-b border-gray-100 dark:border-slate-800 sticky top-0 z-20 transition-colors">
          ...
          <div>
            <h3 className="text-xl font-black text-gray-800 dark:text-white tracking-tight">{t.cashFlow}</h3>
            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">{language === 'en' ? 'Short, medium and long term view' : 'Kısa, orta ve uzun vadeli görünüm'}</p>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar p-5 sm:p-6 space-y-5">
          ...
          <section className="bg-white dark:bg-slate-900 rounded-[32px] p-6 border border-gray-100 dark:border-slate-800 shadow-sm">
            <div className="flex items-start justify-between gap-4 mb-6">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">{t.selectedPeriod}</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-3xl bg-green-50 dark:bg-green-900/20 p-4 border border-green-100 dark:border-green-900/30">
                <p className="text-[10px] font-black uppercase tracking-widest text-green-500 dark:text-green-400 mb-1">{t.income}</p>
              ...
              <div className="rounded-3xl bg-red-50 dark:bg-red-900/20 p-4 border border-red-100 dark:border-red-900/30">
                <p className="text-[10px] font-black uppercase tracking-widest text-red-400 dark:text-red-300 mb-1">{t.expense}</p>
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between px-1">
              <h4 className="text-sm font-black text-slate-700 dark:text-slate-200">{t.movements}</h4>
              <span className="text-[10px] font-bold text-gray-400">{cashFlowSelectedRange.items.length} {t.record}</span>
            </div>
            ...
            {Object.entries(cashFlowGroupedItems).map(([dateKey, items]) => (
              <div key={dateKey} className="space-y-2">
                ...
                {items.map(item => {
                  ...
                  const categoryMeta = getCategoryMeta(item.category || inferTransactionCategory(item.description, item.type), item.type)
                  return (
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          {item.source === 'recurring' && (
                            <span className="px-2 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 text-[9px] font-black uppercase tracking-wide">{t.planned}</span>
                          )}
                          {isIncome && (
                            <span className="px-2 py-0.5 rounded-full bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-300 text-[9px] font-black uppercase tracking-wide">{t.income}</span>
                          )}
                          {!isIncome && (
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wide border ${categoryMeta.className}`}>{categoryMeta.label}</span>
                          )}
                        </div>
                        <p className="text-[11px] font-bold text-gray-400">{account?.name || t.noAccount}</p>
                      </div>
            ...
            {cashFlowSelectedRange.items.length === 0 && (
              <div className="bg-white/70 dark:bg-slate-900/70 rounded-3xl p-6 border border-gray-100 dark:border-slate-800 text-center">
                <p className="text-sm font-bold text-gray-400">{t.noCashFlow}</p>
              </div>
            )}
          </section>
        </main>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-black text-gray-800 dark:text-white tracking-tight">
                {selectedMonthDetail ? t.transactionDetails : t.periodSummary}
              </h3>
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">
                {selectedMonthDetail ? t.personExpenses : t.monthlyHistory}
              </p>
            </div>
          </div>
            ...
            <>
              {monthlyBreakdown
                ...
                .map(item => {
                  ...
                  const monthName = dateLabel(dateObj, { month: 'long', year: 'numeric' });
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center gap-2 mb-4 relative z-10 text-center">
                        <div className="flex flex-wrap shadow-sm justify-center items-center gap-2">
                          ...
                          {isCurrent && <span className="text-[10px] font-bold px-2 py-0.5 bg-white/20 text-white rounded-full backdrop-blur-md">{t.currentMonth}</span>}
                          {isFuture && <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isCurrent ? 'bg-white/10 text-white' : 'bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400'}`}>{t.future}</span>}
                        </div>
              ...
              {isAuthorized(selectedMonthDetail.selectedUserId) && (
                <div className="mb-4">
                  ...
                  >
                    ...
                    <span className="font-bold">{t.addExpense}</span>
                  </button>
                </div>
              )}
              ...
              <div className="space-y-3">
                {(() => {
                  ...
                  if (transactions.length === 0) {
                    return (
                      <div className="text-center py-10 text-gray-400">
                        <p className="text-sm">{t.noExpense}</p>
                      </div>
                    )
                  }
                  ...
                  return transactions.map(t => {
                    ...
                    const categoryMeta = getCategoryMeta(getStoredTransactionCategory(t), t.type);
                    return (
                      <div key={t.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <div className="flex items-center gap-2 text-xs text-gray-400">
                            <span>{dateLabel(t.date, { day: 'numeric', month: 'short', year: 'numeric' })}</span>
              ...
              <form onSubmit={handleAddSubscription} className="space-y-4 mb-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wide pl-2">{t.paymentName}</label>
                    <input type="text" className="w-full p-4 bg-gray-50/50 dark:bg-slate-800 text-gray-800 dark:text-white font-bold rounded-2xl border border-gray-200 dark:border-slate-700 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all" placeholder={subscriptionPaymentType === 'debt' ? (language === 'en' ? 'Mortgage, car loan...' : 'Ev kredisi, araç kredisi...') : (language === 'en' ? 'Netflix, rent, dues...' : 'Netflix, kira, aidat...')} value={subscriptionName} onChange={e => setSubscriptionName(e.target.value)} />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wide pl-2">{t.monthlyAmount}</label>
                  ...
                  <div className="col-span-2">
                    <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wide pl-2">{t.paymentDay}</label>
                  ...
                  {subscriptionPaymentType === 'debt' && (
                    <>
                      <div>
                        <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wide pl-2">{t.totalInstallments}</label>
                      ...
                      <div>
                        <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wide pl-2">{t.paid}</label>
                  ...
                  <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wide pl-2">{t.account}</label>
                    ...
                    {activeAccounts.length === 0 && <option value="">{t.selectCardFirst}</option>}
                  ...
                  {subscriptions.filter(item => item.status !== 0).map(item => {
                    ...
                    return (
                      <div key={item.id} className="bg-gray-50 dark:bg-slate-800 rounded-2xl p-4 border border-gray-100 dark:border-slate-700 flex items-center justify-between gap-3">
                        ...
                        <p className="text-[11px] font-bold text-gray-400">{paymentType.label} - {account?.name || t.noAccount} - {t.everyMonth} {item.dueDay || 15}</p>
                          {installmentInfo && (
                            <p className="text-[11px] font-bold text-gray-500 dark:text-gray-300 mt-1">
                              {installmentInfo.nextInstallment}/{installmentInfo.total} taksit - Kalan {installmentInfo.remainingAfterThisMonth} - {money(installmentInfo.remainingDebt, { maximumFractionDigits: 0 })}
                            </p>
                          )}
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
        ...
        {showAddModal && (
          <div className="absolute inset-0 z-[110] flex items-end sm:items-center justify-center pointer-events-none">
            ...
            <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl w-full sm:w-[420px] h-[92vh] md:h-[85vh] md:max-h-[850px] rounded-t-[40px] sm:rounded-[40px] p-6 sm:p-8 relative z-10 animate-slide-up shadow-2xl flex flex-col pointer-events-auto border border-white/50 dark:border-slate-800/50 transition-colors overflow-x-hidden">
              ...
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h3 className="text-2xl font-black text-gray-800 dark:text-white tracking-tight transition-colors">{editingTransaction ? t.editTransaction : t.newTransaction}</h3>
                  <p className="text-sm text-gray-500 font-medium">{transactionStep === 1 ? t.enterAmount : t.setDetails}</p>
                </div>
                  ...
                  {transactionStep === 1 ? (
                    ...
                    <div className="animate-slide-up space-y-6">
                      <div className="flex flex-wrap items-center gap-2 mb-4" onClick={() => setTransactionStep(1)}>
                        <span className="text-3xl font-black text-indigo-600 dark:text-indigo-400">{money(parseMoneyInput(amount) || 0)}</span>
                        {isInstallment && (
                        ...
                        <div>
                          ...
                          <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-1">
                            {expenseCategories.map(category => {
                              const isSelected = transactionCategory === category.id
                              return (
                                <button
                                  key={category.id}
                                  type="button"
                                  onClick={() => { setTransactionCategory(category.id); setCategoryTouched(true); }}
                                  className={`px-3 py-2 rounded-2xl text-xs font-black border whitespace-nowrap transition-all ${isSelected ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm' : 'bg-gray-50 dark:bg-slate-800 text-gray-500 dark:text-gray-300 border-gray-100 dark:border-slate-700'}`}
                                >
                                  {language === 'en' ? category.en : category.tr}
                                </button>
                              )
                            })}
                          </div>
                                ...
                                <div className="space-y-4">
                                  <div className="flex justify-between items-baseline">
                                    ...
                                    <div className="text-right">
                                      <span className="text-2xl font-black text-gray-900 dark:text-white tracking-tighter">
                                        {money(installmentAmountMode === 'monthly' ? parseMoneyInput(amount) : parseMoneyInput(amount) / installmentCount)}
                                      </span>
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-gray-600 dark:text-gray-400 font-bold text-sm">
                                      {money(installmentAmountMode === 'monthly' ? parseMoneyInput(amount) * installmentCount : parseMoneyInput(amount))}
                                    </span>
                                  </div>
                                </div>
                              </div>
        ...
        {showDeleteConfirmModal && !showIncomeModal && (
          <div className="absolute inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl w-full max-w-[360px] rounded-[40px] p-8 relative z-10 animate-scale-up shadow-2xl border border-white/50 dark:border-slate-800/50">
              <div className="text-center mb-8">
                <div className="w-20 h-20 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center mx-auto mb-4"><Trash2 size={40} className="text-red-500" /></div>
                <h3 className="text-2xl font-black text-gray-800 dark:text-white mb-3">{language === 'en' ? 'Delete Transaction?' : 'İşlemi Sil?'}</h3>
                <p className="text-base text-gray-600 dark:text-gray-400 leading-relaxed">{language === 'en' ? 'Are you sure you want to delete this transaction?' : 'Bu işlemi silmek istediğinize emin misiniz?'}</p>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setShowDeleteConfirmModal(false)} className="flex-1 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-gray-300 py-4 rounded-2xl font-bold transition-all hover:bg-gray-200 dark:hover:bg-slate-700">{t.cancel}</button>
                <button onClick={confirmDeleteTransaction} className="flex-1 bg-gradient-to-r from-red-600 to-red-500 text-white py-4 rounded-2xl font-bold shadow-lg hover:shadow-xl transition-all active:scale-95">{t.delete}</button>
      ...
      <div className="w-full max-w-[480px] md:max-w-[760px] bg-[#F8FAFC] dark:bg-slate-900 h-screen md:h-[92vh] md:max-h-[1000px] md:rounded-[40px] shadow-2xl overflow-hidden relative flex flex-col md:border-[8px] md:border-white dark:md:border-slate-800 ring-1 ring-black/5 z-10 transition-colors duration-300">
        ...
        >
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 md:gap-3">
                ...
                >
                  ...
                  <span className="text-[10px] font-black uppercase tracking-tight">{t.addExpense}</span>
                </button>
              </div>
              ...
              {(() => {
                ...
                return (
                  <div className="relative bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-2 md:px-3 py-1.5 rounded-xl border border-indigo-100 dark:border-indigo-800/50 flex flex-col items-end shadow-sm shrink-0">
                    ...
                    <span className="text-[8px] font-black uppercase mb-0.5 tracking-wider">{t.daysToNewPeriod}</span>
                    <span className="text-xs font-black leading-none">{daysLeft === 0 ? t.tomorrow : `${daysLeft} ${t.days}`}</span>
                  </div>
                );
              })()}
          ...
          {isMenuOpen && (
            <div className="mx-6 mb-6 p-6 bg-white dark:bg-slate-800 rounded-[32px] shadow-xl border border-white dark:border-slate-700 animate-in fade-in slide-in-from-top-4 duration-300">
              <div className="flex justify-between items-center mb-6 px-1">
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">{t.quickActions}</h3>
                <button
                  onClick={() => {
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 sm:gap-4 justify-items-center">
                {menuOrder.map(itemId => {
                  ...
                  switch (itemId) {
                    ...
                    case 'future': label = t.periods; IconComponent = Calendar; break;
                    case 'cashflow': label = t.cashFlow; IconComponent = BarChart3; break;
                    case 'subscriptions': label = t.recurringPayments; IconComponent = Repeat; break;
                    case 'cards': label = language === 'en' ? 'Cards' : 'Kartlar'; IconComponent = CreditCard; break;
                    case 'users': label = t.people; IconComponent = Users; break;
                    ...
                    case 'extract': label = t.statement; IconComponent = Receipt; break;
                    case 'portfolio': label = t.portfolio; IconComponent = Wallet; colorClass = 'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-600 dark:text-yellow-400'; borderColorClass = 'border-yellow-100 dark:border-yellow-900/30'; break;
                    case 'settings': label = t.management; IconComponent = Settings; break;
                    default: return null;
                  }
            ...
            >
              ...
              <div className="bg-white dark:bg-slate-900 w-full max-w-[380px] max-h-[calc(100dvh-2.5rem)] rounded-[32px] sm:rounded-[36px] p-6 relative z-10 animate-scale-up shadow-2xl border border-white/50 dark:border-slate-800/50 overflow-hidden">
                <div className="flex items-center justify-between mb-5">
                  <div>
                    <h3 className="text-xl font-black text-gray-800 dark:text-white tracking-tight">{t.upcomingPayments}</h3>
                    <p className="text-xs text-gray-400 font-bold">{t.nextFiveDays}</p>
                  </div>
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="px-2 py-0.5 rounded-full bg-white dark:bg-slate-800 text-amber-600 dark:text-amber-300 text-[9px] font-black uppercase">
                            {item.daysLeft === 0 ? t.today : item.daysLeft === 1 ? t.tomorrow : `${item.daysLeft} ${t.days.toLowerCase()}`}
                          </span>
                        </div>
                        <p className="text-[11px] font-bold text-gray-400">{item.user?.name || t.general} - {item.account?.name || t.noAccount}</p>
                      </div>
                      <p className="font-black text-amber-700 dark:text-amber-300 shrink-0">
                        {money(item.amount, { maximumFractionDigits: 0 })}
                      </p>
                    </div>
                  ))}
                </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-end mb-4">
                      <div>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">{t.spent}</p>
                        <p className={`text-3xl font-black tracking-tight ${isLimitExceeded ? 'text-red-500' : 'text-gray-800 dark:text-white transition-colors'}`}>
                          {money(userSpending, { maximumFractionDigits: 0 })}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">{t.remaining}</p>
                        <p className="text-lg font-bold text-gray-600 dark:text-gray-300 transition-colors">
                          {money(remaining, { maximumFractionDigits: 0 })}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                      ...
                      <span>Limit: {money(userLimit, { maximumFractionDigits: 0 })}</span>
                    </div>
                  </div>
                )
          ...
          <div className="mx-6 mb-8">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 px-1">{t.investment}</h3>
            <div
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="font-bold text-gray-800 dark:text-white text-lg transition-colors">{t.portfolio}</span>
                    <p className="text-[10px] text-gray-400 font-medium italic mt-0.5">{t.portfolioSub}</p>
                  </div>
                </div>
              ...
              <div className="flex justify-between items-end">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-gray-400 text-xs font-bold uppercase tracking-wider">{t.totalAsset}</p>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <p className="text-3xl font-black tracking-tight text-gray-800 dark:text-white transition-colors">
                    {isBalanceHidden
                      ? '••••••'
                      : money(portfolio.lastTotal, { maximumFractionDigits: 0 })
                    }
                  </p>
                </div>
                <div className="text-right">
                  {lastUpdateTime && (
                    <div className="flex flex-col items-end">
                        <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest mb-0.5">{t.liveRate}</p>
                    ...
                    <div className="animate-slide-up space-y-6">
                      <div className="flex flex-wrap items-center gap-2 mb-4" onClick={() => setTransactionStep(1)}>
                        <span className="text-3xl font-black text-indigo-600 dark:text-indigo-400">
                          {money(parseMoneyInput(amount) || 0)}
                        </span>
                        {isInstallment && (
                          <span className="text-[10px] bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 px-2 py-1 rounded-lg font-black uppercase">
                            {installmentAmountMode === 'monthly' ? 'Taksit Tutarı' : 'Toplam Tutar'}
                      ...
                      <div className="space-y-6 mb-6">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={description}
                            onChange={e => { setDescription(e.target.value); if (!categoryTouched) setTransactionCategory(inferTransactionCategory(e.target.value)); }}
                          />
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-gray-400 mb-2.5 uppercase tracking-wide pl-2">{language === 'en' ? 'Category' : 'Kategori'}</label>
              ...
              <div className="space-y-3 pr-2">
                ...
                <div className="grid grid-cols-3 gap-2 mb-5">
                  <div className="bg-green-50 dark:bg-green-900/20 rounded-2xl p-3 border border-green-100 dark:border-green-900/30">
                    ...
                    <p className="text-sm font-black text-green-700 dark:text-green-300">{money(monthlyReport.income, { compact: true })}</p>
                  </div>
                  <div className="bg-red-50 dark:bg-red-900/20 rounded-2xl p-3 border border-red-100 dark:border-red-900/30">
                    ...
                    <p className="text-sm font-black text-red-600 dark:text-red-300">{money(monthlyReport.expenses, { compact: true })}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-3 border border-slate-100 dark:border-slate-700">
                    ...
                    <p className={`text-sm font-black ${monthlyReport.net >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>{money(monthlyReport.net, { compact: true })}</p>
                  </div>
                </div>
                  ...
                  {monthlyReport.userBreakdown.length > 0 && (
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <p className="text-[10px] font-bold text-gray-400">{money(monthlyReport.recurringExpenses, { maximumFractionDigits: 0 })} planlı</p>
                      </div>
                          ...
                          return (
                            <div key={item.user.id} className="space-y-1">
                              <div className="flex justify-between text-xs font-bold">
                                ...
                                <span className="text-gray-400">{money(item.total, { maximumFractionDigits: 0 })}</span>
                              </div>
                        ...
                        {monthlyReport.topTransactions.map(item => {
                          ...
                          return (
                            <div key={`top-${item.id}`} className="flex items-center justify-between gap-3 text-xs">
                              ...
                              <span className="font-black text-red-500 dark:text-red-400 shrink-0">{money(item.amount, { maximumFractionDigits: 0 })}</span>
                            </div>
                          )
                        })}
                      </div>
                  ...
                  .map(t => {
                    ...
                    return (
                      <div key={t.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm hover:scale-[1.01] transition-transform print-transaction">
                        ...
                        <p className="text-xs text-gray-400">{dateLabel(t.date, { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                      </div>
                    )
                  })}
