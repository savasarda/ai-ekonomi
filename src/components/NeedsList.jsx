...
const NeedsList = ({ onBack, isSupabaseConfigured, language = 'tr' }) => {
    ...
    const requestDeleteCompleted = () => {
        if (!needs.some(n => n.completed)) return;
        setShowDeleteCompletedConfirm(true);
    };

    const handleDeleteCompleted = async () => {
        const completedIds = needs.filter(n => n.completed).map(n => n.id);
        if (completedIds.length === 0) return;

        const originalNeeds = [...needs];
        const updated = needs.filter(n => !n.completed);
        setShowDeleteCompletedConfirm(false);
        setNeeds(updated);

        if (isSupabaseConfigured) {
            try {
                const { error } = await supabase.from('needs_list').delete().in('id', completedIds);
                if (error) throw error;
            } catch (e) {
                console.error('Error deleting completed needs:', e);
                setNeeds(originalNeeds);
                alert(t.error + e.message);
            }
        } else {
                    ...
                    {!isLoading && needs.some(n => n.completed) && (
                        <div className="mt-8">
                            <div className="flex items-center justify-between px-2 mb-3">
                                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t.completed}</h3>
                                <button
                                    type="button"
                                    onClick={requestDeleteCompleted}
                                    title={language === 'en' ? 'Delete completed' : 'Tamamlananları sil'}
                                    aria-label={language === 'en' ? 'Delete completed needs' : 'Tamamlanan ihtiyaçları sil'}
                    ...
                    <div className="w-full max-w-sm rounded-[28px] bg-white p-5 shadow-2xl shadow-slate-900/20 ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
                        <div className="mb-5 flex items-start gap-4">
                            ...
                            <div className="min-w-0">
                                <h2 className="text-lg font-black text-slate-900 dark:text-white">
                                    {language === 'en' ? 'Delete completed?' : 'Tamamlananlar silinsin mi?'}
                                </h2>
                                <p className="mt-1 text-sm font-medium leading-relaxed text-slate-500 dark:text-slate-400">
                                    {language === 'en'
                                        ? 'Completed needs will be removed from the list.'
                                        : 'Tamamlanan ihtiyaçlar listeden kaldırılacak.'}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 rounded-2xl bg-slate-100 px-4 py-3 text-sm font-black text-slate-600 transition-colors hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
                            >
                            ...
                            <button
                                ...
                                className="flex-1 rounded-2xl bg-red-500 px-4 py-3 text-sm font-black text-white shadow-lg shadow-red-500/25 transition-colors hover:bg-red-600"
                            >
